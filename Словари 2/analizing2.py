def exceptString(string): return string.replace(",", "").replace(".", "").replace("!", "").replace("?", "").replace(":", "").replace(";", "")
dictwords, words = {}, []
for i in range(int(input())): words += [exceptString(word.lower()) for word in input().split()]
